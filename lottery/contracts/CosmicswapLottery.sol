...
contract CosmicswapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInCosmic = 50 ether;
    uint256 public minPriceTicketInCosmic = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInCosmic;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] cosmicPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInCosmic;
        uint32 finalNumber;
    }
     ...
     * @param _cosmicTokenAddress: address of the COSMIC token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _cosmicTokenAddress, address _randomGeneratorAddress) {
        cosmicToken = IERC20(_cosmicTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of COSMIC to this contract
        uint256 amountCosmicToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInCosmic,
            _ticketNumbers.length
        );

        // Transfer cosmic tokens to this contract
        cosmicToken.safeTransferFrom(address(msg.sender), address(this), amountCosmicToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInCosmic += amountCosmicToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInCosmicToTransfer
        uint256 rewardInCosmicToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInCosmicToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        cosmicToken.safeTransfer(msg.sender, rewardInCosmicToTransfer);

        emit TicketsClaim(msg.sender, rewardInCosmicToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in COSMIC per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInCosmic) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in COSMIC for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].cosmicPerBracket[j] =
                ...
                // A. No COSMIC to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].cosmicPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInCosmic - amountToShareToWinners);

        // Transfer COSMIC to treasury address
        cosmicToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in COSMIC token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        cosmicToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInCosmic += _amount;
     ...
     * @param _priceTicketInCosmic: price of a ticket in COSMIC
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInCosmic >= minPriceTicketInCosmic) && (_priceTicketInCosmic <= maxPriceTicketInCosmic),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInCosmic: _priceTicketInCosmic,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            cosmicPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInCosmic: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(cosmicToken), "Cannot be COSMIC token");
    ...
    /**
     * @notice Set COSMIC price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInCosmic: minimum price of a ticket in COSMIC
     * @param _maxPriceTicketInCosmic: maximum price of a ticket in COSMIC
     */
    function setMinAndMaxTicketPriceInCosmic(uint256 _minPriceTicketInCosmic, uint256 _maxPriceTicketInCosmic)
        external
        onlyOwner
    {
        require(_minPriceTicketInCosmic <= _maxPriceTicketInCosmic, "minPrice must be < maxPrice");

        minPriceTicketInCosmic = _minPriceTicketInCosmic;
        maxPriceTicketInCosmic = _maxPriceTicketInCosmic;
    }
     ...
     * @param _priceTicket price of a ticket (in COSMIC)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].cosmicPerBracket[_bracket];
        } else {
            return 0;
        }
    }
