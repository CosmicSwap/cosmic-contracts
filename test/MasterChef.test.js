...
contract('MasterChef', ([alice, minter]) => {
    beforeEach(async () => {
        ...
        this.chef = await MasterChef.new(this.cosmic.address, '10', { from: minter });
    });
    it('update emission rate', async () => {
        assert.equal((await this.chef.startBlock()).valueOf(), '10');
        assert.equal((await this.chef.cosmicPerBlock()).valueOf(), '1000000000000000000');
        ...
        assert.equal((await this.chef.cosmicPerBlock()).valueOf(), '970000000000000000');
        ...
        assert.equal((await this.chef.cosmicPerBlock()).valueOf(), '858734025700000000');
        ...
        assert.equal((await this.chef.cosmicPerBlock()).valueOf(), '101831014379039644');
        ...
        assert.equal((await this.chef.cosmicPerBlock()).valueOf(), '100000000000000000');
